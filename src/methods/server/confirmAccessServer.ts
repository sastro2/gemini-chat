...
export const confirmAccessServer = async(cookie: string | undefined, res: NextApiResponse<DefaultApiResponseBody>, resBody: DefaultApiResponseBody, username?: string): Promise<number> => {
  const accessOptions = await validateAccessOptions(cookie, res, true, username);
