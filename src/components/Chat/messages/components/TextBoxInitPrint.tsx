...
export const TextBoxInitPrint: React.FC<ITextBoxInitPrint> = (props) => {
  ...
  return(
    <Container key={message.parts + index} style={{display: 'flex', justifyContent: message.role === 'model'? 'start':  'end', padding: '0.5%', position: 'relative'}} maxWidth={false}>
      ...
      <Container style={{border: `${StyleSheet.background.messages.message} 0.01px solid`, borderRadius: message.role === 'model'? '2px 15px 15px 15px':  '15px 3px 15px 15px', width: 'auto', padding: '0 3% 0 3%', margin: 0, color: StyleSheet.characters, backgroundColor: StyleSheet.background.messages.message, letterSpacing: '0.3px', boxShadow: StyleSheet.shadow}} maxWidth={false}>
